...
Let's suppose I have an API gateway (a glorified proxy, in this case) protecting requests destined for downstream microservice APIs. In the OAuth world, the gateway would act as an OAuth client, and the API would act as a resource server. Both an OAuth client and an OAuth resource server reject requests without a valid OAuth token. The OAuth client can help you originate a token, however, where the resource server will reject requests outright. The OAuth client involves an OAuth authorization service to handle the login and authentication. An authorization service is one of many different identity providers (IDPs), like Spring Authorization Server, Active Directory, Okta/Auth0, and Keycloak. Once the user has authenticated, the authorization service returns control to the OAuth client which in turn now has a token it can send along with the proxied requests destined for the downstream API.
